...
export class NgxSimpleCountdownDirective implements OnInit, OnDestroy {
  ...
  private initSimpleCountdown(): void {
    ...
    this.interval$ = interval(1000).pipe(
      ...
      tap((_) => (this.countdownResult = this.updateSimpleCountdown(this.totalSecondes))),
  ...
  private createHTML(): void {
    ...
    if (this.totalSecondes > 0) {
      ...
      if (this.isContent('m')) {
        if (
          (this.countdownResult.minutes > 0 || this.countdownResult.hours > 0 || this.countdownResult.day > 0) &&
          this.reactive
        ) {
          o += `<div style="${this.styles}">
